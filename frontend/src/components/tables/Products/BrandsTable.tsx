...
const BrandsTable = () => {
  ...
  const [products, setProducts] = useState<BrandProps[]>([])
  const [targetSelection, setTargetSelection] = useState<any | undefined>(
    undefined
  )
  const [editBrand, setEditBrand] = useState<BrandProps | undefined>(undefined)
  ...
  const handlerDelete = async () => {
    ...
    try {
      const response = await AuthSending().delete(
        `${API_ENDPOINT}${API_VERSION}brands/delete/${targetSelection?.id}`
      )
        ...
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {paginatedUsers.length > 0 ? (
              paginatedUsers.map((brands, index) => (
                <TableRow key={index} className="text-nowrap">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 overflow-hidden rounded-full">
                        <img
                          ...
                          src={
                            brands.logo_url
                              ? `${PUBLIC_STATIC}brands/${brands.logo_url}`
                              : "/blank-profile.png"
                          }
                      ...
                      <div>
                        ...
                        <span className="text-theme-xs block text-gray-500 dark:text-gray-400">
                          {brands.website_link ? (
                            <Link to={brands.website_link} target="_blank">
                  ...
                  <TableCell className="text-theme-xs px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                    {brands.brand_description ?? "ไม่พบข้อมูล"}
                  </TableCell>
                  <TableCell className="text-theme-sm px-4 py-3 text-gray-500 dark:text-gray-400">
                    <div className="inline-flex gap-1">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          openModal(), setTargetSelection(brands)
                        }}
                      >
                        <TrashBinIcon />
                      </Button>
