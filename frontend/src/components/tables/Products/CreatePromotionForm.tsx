...
const CreatePromotionForm: React.FC<CreatePromotionProps> = ({
  reFetching,
  onClose,
  editData,
}) => {
  ...
  useEffect(() => {
    if (editData) {
      setFormData({
        name: editData.name || "",
        unit: editData.link || "", // adapt if `website_link` is available
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editData
        ? `${API_ENDPOINT}${API_VERSION}promotions/update/${editData.id}`
        : `${API_ENDPOINT}${API_VERSION}promotions/create`
    ...
    <div className="no-scrollbar relative w-full overflow-y-auto rounded-3xl bg-white p-4 lg:p-11 dark:bg-gray-900">
      <div className="px-2 pr-14">
        <h4 className="mb-6 text-2xl font-semibold text-gray-800 dark:text-white/90">
          {editData ? "แก้ไขโปรโมชั่น" : "เพิ่มโปรโมชั่น"}
        </h4>
      </div>
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
              ...
              <div className="col-span-2 lg:col-span-1">
                ...
                <Input
                  ...
                  error={error.name}
                  hint={error.name ? "โปรดระบุหัวข้อโปรโมชั่น" : ""}
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <TextArea
                  ...
                  hint={error.description ? "โปรดระบุรายละเอียดโปรโมชั่น" : ""}
                />
              </div>
            </div>
          </div>
