...
interface DECiTProjectFormProps {
  mode: "create" | "edit"
  existingData?: DECiTProjectProps
}

const DECiTProjectForm: React.FC<DECiTProjectFormProps> = ({
  mode,
  existingData,
}) => {
  const navigate = useNavigate()

  const [decitData, setDecitData] = useState({
    name: "",
    description: "",
  })
  ...
  const handlerChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setDecitData(prev => ({ ...prev, [name]: value }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!decitData.name || !decitData.description) {
    ...
    try {
      ...
      formData.append("name", decitData.name)
      formData.append("description", decitData.description)
      ...
      if (mode === "edit" && existingData) {
        res = await UPDATE_DECIT_PROJECT(Number(existingData.id), formData)
      } else {
        res = await CREATE_DECIT_PROJECT(formData)
      }
    ...
    <div className="top-24 z-20 col-span-2 mx-auto h-fit max-w-3xl rounded-2xl border border-gray-200 bg-white transition duration-200 xl:sticky xl:col-span-1 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="px-6 py-5">
        <h3 className="text-2xl font-medium text-gray-800 dark:text-white/90">
          {mode === "create"
            ? "ฟอร์มเพิ่มผลงานช่างมือ DECiT"
            : "ฟอร์มแก้ไขผลงานช่างมือ DECiT"}
        </h3>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div className="col-span-2 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                {previewImage ? (
                  ...
                  <ResponsiveImage
                    src={`${getImageUrl("projects", existingData.image)}`}
            ...
            <div className="col-span-2 flex items-center justify-between">
              <Button
                onClick={() => navigate("/decit-projects")}
                variant="outline"
                isButton
...
export default DECiTProjectForm
