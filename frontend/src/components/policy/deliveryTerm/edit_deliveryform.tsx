...
interface EditDeliveryFormProps {
  isOpen: boolean
  deliveryData: DeliveryTermProp
  onUpdate: (updated: DeliveryTermProp) => void
  onClose: () => void
}

const EditDeliveryForm: React.FC<EditDeliveryFormProps> = ({
  deliveryData,
  onUpdate,
  onClose,
}) => {
  const [delivery_term_title, setTitle] = useState(
    deliveryData.delivery_term_title
  )
  const [delivery_term_content, setContent] = useState(
    deliveryData.delivery_term_content
  )
